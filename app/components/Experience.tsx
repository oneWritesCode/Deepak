...
function ExperienceComponent({
  ...
  return (
    <div className="bg-Dark rounded-2xl max-w-100 bg-black text-white p-4 border border-gray-800">
        <div className="pl-1 mb-2 sourGummy uppercase flex items-center justify-start gap-2">
        <span className="text-md md:text-2xl font-bold">{roleWas}</span> 
        <span> • </span>
        <span className="text-xs text-green-300"> {jobType}</span> 
        </div>

      <div className="w-full h-10 md:h-12 pb-2 flex items-center justify-between gap-10">
        <div className="h-full  gap-2 flex items-center justify-center">
          ...
          />
          <p className="text-md md:text-xl sourGummy uppercase text-gray-300 font-semibold">{workingAt}</p>
        </div>
      ...
      <div>
        <p className="text-sm text-gray-100 pl-px sourGummy font-medium">{workingSince} -
             {workedSince=== "current"? <span className="text-green-300"> {workedSince}</span>:" "}
              </p>

        <p className="text-sm pl-px pt-1 text-gray-400 sourGummy line-clamp-3">{descriptionOfTheRole}</p>
      </div>
    </div>
  );
}
